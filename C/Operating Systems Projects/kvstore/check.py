...
def server_tests():
    return [
        ...
        TestGroup('Concurrent SET', 'concset', 1,
                  ...
                  Test('pthread locking', test_set_locking, stop_group_on_fail=True),
                  ),
        TestGroup('Concurrent GET', 'concget', 1,
                  ...
                  Test('pthread locking', test_get_locking, stop_group_on_fail=True)
                  ),
        TestGroup('R/W lock', 'rwlock', 2,
                  Test('Concurrent GETs', test_rwlock_get),
                  Test("POSIX wrlock and rdlock", test_rw_locking),
                  ),
        TestGroup('Thread pool', 'pool', 2,
                  Test('Has thread pool', test_threadpool),
                  Test('Are threads synchronized', test_threadpool_sync, stop_group_on_fail=True),
                  ),
        TestGroup('Stress', 'stress', 1,
        ...
        TestGroup('STAT', 'stat', 1,
                  Test('get statistics', get_statistics),
                  ),
    ]
...
#
# @param ts: TestSetup()
# Helper function. used in test_write_locking and test_read_locking
#
def set_ops(ts):
    for _ in range(0, 10):
        key = randstr(4, 10)
        value = randstr(4, 16)
        ts.set(key, value, allow_error=True)
        ...
        for test in self.tests:
            ...
            try:
                ...
                if self.stop_if_fail:
                    break
                if test.stop_group_on_fail:
                    output.write('You failed a critical test. Your points are zeroed for this test group', color='red')
                    succeeded = 0
                    break
            else:
                ...
                if ltype == 'B':
                    ...
                    try:
                        ...
                        raise TestError(f'Error parsing {SERVER_DUMP}: '
                                        f'Line: {line} is malformed (missing field)') \
...
class Client:
    ...
    def send(self, data):
        ...
        try:
            ...
            raise TestError('Server closed connection while sending data (socker error).') from e
    ...
    def recvline(self):
        while b'\n' not in self._recvbuf:
            try:
                ...
                extra = ''
                if self.last_cmd in ('DUMP', 'SETOPT', 'PING'):
...
class TestSetup:
    ...
    def __enter__(self):
        self.server_stack = ExitStack()
        self.client_stack = ExitStack()
        try:
            self.server_stack.enter_context(self.server)
            for client in self.clients:
                self.client_stack.enter_context(client)
        except Exception as e:
            self.client_stack.close()
    ...
    def stat(self, key, client=0, allow_error=False):
        client = self.clients[client]
        err = None
        try:
            value = client.cmd('STAT', key)
        except ServerError as e:
            if not allow_error:
                raise e
            else:
                ...
                err = e
        if err:
            raise TestError(f'Server error for STAT {key} '
                            f'that should exist')
...
def test_threads_cleanup():
    ...
    with Server() as server:
        ...
        if basethreads > 1: # one thread for logging
            raise TestError(f'Server has {basethreads-1} threads at start, '
        ...
        with ExitStack() as stack:
            ...
            if loadthreads < NCLIENTS-1:
                raise TestError(f'After launching {NCLIENTS} clients, we '
                                f'expect {NCLIENTS} threads, but only '
                                f'{loadthreads-1} threads were found (and no '
                                f'threadpool was detected either, because: '
                                f'{threadpool_error})')

        # Give server some time to process all disconnects and clean up threads.
        time.sleep(0.1)
...
def check_log(op):
    with TestSetup() as ts:
        time.sleep(0.2)  # wait for the server to be create log file
        ...
        for line in file:
            ...
            primitive = s[0]
            address = s[1]
            if "unlock" in primitive or "wrunlock" in primitive:
                if address not in locked:
                    raise TestError(f"Unlocking mutex not locked (address:{address})")
                else:
                    locked[address] = 0
            elif "lock" in primitive or "rwlock" in primitive:
                locked[address] = 1
            elif "pthread_cond_wait" in primitive:
                if address not in locked:
                    raise TestError(f"Mutex must be locked to call pthread_cond_wait (mutex address: :{address})")
                locked[address] += 1

        if len(locked) == 0:
            raise TestError('You are not using pthread locks!')

        for key in locked.keys():
            if locked[key] != 0:
                raise TestError(f'Mutex at {key} not unlocked after locking')
        return len(locked)


def test_get_locking():
    def op(ts):
        validkeys = set()
        for _ in range(10):
        ...
        for _ in range(0, 100):
            ...
            ts.get(key, allow_error=True)

    os.remove(SERVER_LOG)
    num_locks = check_log(op)
    if num_locks < 2:
        raise TestError("You are using 1 mutex only. You should not lock the whole hashtable.")


def test_set_locking():
    def op(ts):
        for _ in range(0, 100):
            key = randstr(4, 10)
            value = randstr(4, 16)
...
def test_rw_locking():
    ...
    with TestSetup() as ts:
        ...
        for line in file:
            ...
            primitive = s[0]
            address = s[1]
            if "rwunlock" in primitive:
                if address not in locked:
                    raise TestError(f"Unlocking mutex not locked (address:{address})")
                else:
                    locked[address] = 0
            elif "rdlock" in primitive or "wrlock" in primitive:
                locked[address] = 1

        if len(locked) == 0:
            raise TestError('You should use pthread_rwlock_wrlock and pthread_rwlock_rdlock !')
...
def test_threadpool_sync():
    with Server(reset_on_start=False) as server:
        pids = server.get_threads()
        time.sleep(0.2)  # wait for the server to be create log file
        file = open(SERVER_LOG)
        ...
        for line in file:
            ...
            primitive = s[0]
            address = s[1]
            if "unlock" in primitive or "rwunlock" in primitive:
                if address not in locked:
                    raise TestError(f"Unlocking mutex not locked (address:{address})")
                else:
                    locked[address] = 0
            elif "lock" in primitive or "rdlock" in primitive or "rwlock" in primitive:
                locked[address] = 1
            elif "pthread_cond_wait" in primitive:
                num_pthread_wait += 1
                if address not in locked:
        ...
        if num_pthread_wait != (len(pids) - 1):
            raise TestError(
                f'Server has {len(pids) - 1} workers but they call pthread_cond_wait only {num_pthread_wait} times')
...
def get_statistics():
    ...
    with TestSetup(nclients=10) as ts:
        tot = ts.stress_test(op, duration=5, expected_ops_per_worker=10)
        tot += 2  # dump + rst
        n = ts.stat('REQ', allow_error=False)
        n = int(n)
        if n < tot:
            raise TestError(f'Server reports bad statistics.\n '
                            f'expected #req:{tot}, got:{n}')

        thgExp = n / 5
        thg = ts.stat('THG')
        thg = float(thg)
        var = thgExp * 0.1
        if thg < (thgExp - var) or thg > (thgExp + var):
            raise TestError(f'Server reports bad statistics.\n '
                            f'expected throughput:{thgExp}, got:{thg}')
...
def test_stress_set_random():
    ...
    with TestSetup(nclients=10) as ts:
        ts.stress_test(op, expected_ops_per_worker=10)
